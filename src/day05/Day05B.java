...
public class Day05B {
	...
	public static void main(String[] args) throws IOException {
		BufferedReader in = new BufferedReader(new FileReader("res/Day05Input.txt"));
		...
		while ((line = in.readLine()) != null) {
			...
			// Check if line is diagonal
				...
				if (command[0] < command[2] && command[1] < command[3]) {
					for (int i = 0; i <= command[2] - command[0]; i++) {
						floor[command[0] + i][command[1] + i] += 1;
					}
					
				// Bottom right to top left
				} else if (command[0] > command[2] && command[1] > command[3]) {
					for (int i = 0; i <= command[0] - command[2]; i++) {
						floor[command[0] - i][command[1] - i] += 1;
					}
